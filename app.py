...
from plotnine import ggplot, aes, geom_point, geom_smooth, labs, scale_colour_gradient

with st.echo(code_location='below'):
    st.write("# Проект: Визуализация и анализ данных, связанных с Формулой-1.")
    ...
    Поэтому основные объекты для анализа и визуализации данных про Формулу-1 - это выступления гонщиков и команд в 
    разных гонках, а также во всём сезоне."""
    """Проект можно разделить на несколько частей, в которых используются различные виды визуализации.
    Так как объём используемой базы данных достаточно большой, а проект предполагает большое
    количество интерактивности, то после того, как Вы просмотрите информацию по одной из частей, я советую Вам вернуться 
    к началу этого раздела и скрыть его. В таком случае визуализации для остальных частей будут работать быстрее!"""
    """Наконец, я предлагаю приступить к первой части моего проекта.
    Для этого просто нажмите "Показать Часть 1" ниже."""

    st.write("## Часть 1: Анализ данных по отдельным гонщикам и странам")

    show1 = st.checkbox("Показать Часть 1")

    if show1:
        """Начнём с анализа гонщиков Формулы-1."""
        """Ниже вы можете вписать в окошко полное имя гонщика (имя и фамилию на английском языке) и посмотреть краткую информацию 
        ...
        или найти гонщиков в интернете:\n
        ...
        """Когда Вы посмотрите всё, что хотели в этом разделе, не забудьте скрыть его!"""

    st.write("## Часть 2: Анализ данных по разным странам")

    show2 = st.checkbox("Показать Часть 2")
    ...
    if show2:
        ...
        """Ниже вы можете выбрать какую-либо страну (точнее, национальность) и увидеть, какие гонщики этой национальности становились победителями 
        ...
        if not total_victories.empty:
            ...
            """У этой страны пока нет гонщиков победителей. Выберите другую!"""
    ...
    show3 = st.checkbox("Показать Часть 3")
    ...
    if show3:
        ...
        (1-ое, 2-ое и 3-ье места) и количество побед. Ниже Вы можете выбрать один из этих параметров и посмотреть, 
        какие гонщики являются лучшими по каждому из них."""

        choice = st.radio('Выберите:', ['Количество проведённых гонок', 'Количество подиумов', 'Количество побед'])
        ...
        if choice == 'Количество подиумов':
            ...
            chart2.set_title('The drivers with the highest number of podiums')
            st.pyplot(fig)
        ...
        лишь в одном или двух из них. В связи с этим я предлагаю посмотреть, как связаны между собой количество проведённых 
        Гран-При, количество побед и призовых мест."""
        ...
        Например, за 100 гонок спортсмены в среднем выигрывают около 6-7 гонок и приезжают на подиум приблизительно в 
        20 гонках."""
        ...
        """Этот график показывает взаимосвязь между количеством подиумов и побед гонщиков. Как и ранее, можно предположить,
        сколько раз гонщику в среднем придётся финишировать на подиуме, чтобы добиться какого-то определённого числа побед."""

        """Когда Вы посмотрите всё, что хотели в этом разделе, не забудьте скрыть его!"""

    st.write("## Часть 4: Динамика")

    show4 = st.checkbox("Показать Часть 4")
    ...
    if show4:
        ...
        def animation1():
            ...
            figure = px.bar(data3, x='victories', y="name", color="name",
                            animation_frame="year", labels={"victories": "Victories", "name": "Team", "year": 'Year'},
                    title="The dynamics of victories for top 15 teams", width=700, height=700)
            figure.update_layout(font=dict(size=14))
            return figure
    ...
    show5 = st.checkbox("Показать Часть 5")
    ...
    if show5:
        ...
        с худшими временами). Во втором сегменте происходит то же самое: 5-6 самых медленных гонщиков из оставшихся не 
        проходят дальше. В третьем сегменте решается судьба поул-позиции (права стартовать с первой позиции в гонке).
        За это право сражаются оставшиеся 10 самых быстрых гонщиков."""

        """Далее я предлагаю выбрать любой Гран-При, который прошёл с 2006 года по 2021 год. 
        Для этого сначала выберите год, а затем один из доступных этапов. 
        После этого Вы можете выбрать одного из гонщиков, участвоваших в этой квалификации и посмотреть, как он выступил."""

        """На этой визуализации каждая точка - отдельный пилот и его лучшее время в сегменте. Наведя курсор мышки на точку, 
        Вы можете посмотреть, какому пилоту соответствует эта точка и какое время он показал в данном сегменте. 
        ...
        в гонке на этапе, для которого Вы сейчас смотрели результаты квалификации, то выберите соответствующий вариант ниже.
        ...
        фамилию гонщика справа и тогда будет выделена линия с его выступлением в гонке). Кроме того, что из этой визуализации 
        видно, кто когда и кого обгонял, можно увидеть ещё многое другое. Например, можно проанализировать, у кого получился 
        хороший старт, а у кого плохой (для этого нужно посмотреть, сколько позиций отыграл или проиграл гонщик за первый круг).
        ...
        2012 и Гран-При Германии 2019. Эти гонки получились очень зрелищными и интересными для анализа: множество обгонов, 
        ...
        тем чаще гонщик оказывался на этой позиции. Предлагаю также посмотреть на такую визуализацию на различных примерах."""
        ...
        driver_laps['milliseconds'] = driver_laps['milliseconds'] / 1000

        """На следующей визуализации Вы можете посмотреть, с каким темпом шёл гонщик на протяжении гонки. 
        Точки на этом графике отражают то, за какое время гонщик прошёл тот или иной круг. Как Вы можете видеть, в большинстве
        случаев гонщик проходит гонку в одном темпе. Иногда темп к концу гонки растёт, так как запас топлива сокращается и 
        болид становится легче. Но также есть точки, лежащие сильно выше чёрной кривой, усредняющей темп гонщика. Эти точки 
        ...
        plot4 = (ggplot(driver_laps[lambda x: x['milliseconds'] < 200], aes(x='lap', y='milliseconds'))
                 ...
                 labs(x="Lap", y="Time", title=f"The performance of {the_driver} in the race of {grand_prix} {a}",
                      color="Time (seconds)") +
                 scale_colour_gradient(low='green', mid='yellow', high='red')
                 )
    ...
    show6 = st.checkbox("Показать Часть 6")
    ...
    if show6:
        ...
        заметить как минимум одну тенденцию: количество участников Гран-При уменьшается при приближении к настоящему времени. 
        Это объясняется тем, что сначала Формула-1 была очень непопулярным спортом и гонщиками мог стать любой достаточно 
        богатый человек, который не боялся попасть в аварию на огромной скорости. А сейчас чемпионат Формулы-1 - это 20
        лучших гонщиков планеты, которые проделали огромный путь, чтобы попасть в Формулу-1."""
        ...
        graph1 = alt.Chart(table_of_standings).mark_line(point=True).encode(
            ...
            y=alt.Y("position", scale=alt.Scale(zero=False), sort='descending', axis=alt.Axis(title='Position')),
            color=alt.Color("fullname", legend=alt.Legend(title='The drivers')),
            tooltip=[alt.Tooltip('fullname'), alt.Tooltip('position'), alt.Tooltip('points')],
        ...
        """В заключение, предлагаю посмотреть и на кубок конструкторов. В этот раз для разнообразия обойдёмся без динамики
        на протяжении, а посмотрим лишь на сами итоги и увидим, какая команда стала чемпионом в выбранный год."""

        year = st.slider('Выберите год:', 1958, 2020)
        fig, ax = plt.subplots()
